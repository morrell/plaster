...
$InstallPath = Join-Path (Split-Path $profile.CurrentUserAllHosts -Parent) `
                "Modules\$ModuleName\$((Import-LocalizedData -BaseDirectory $SrcRootDir -FileName "$ModuleName.psd1").ModuleVersion.ToString())"
...
<%
"`$PackageRestoreEnabled = `$$($PLASTER_PARAM_PackageRestore -ne 'None')"
%>
...
Task AfterStageFiles -After StageFiles {
    ...
    If (get-command git -ErrorAction SilentlyContinue) {
        $ReleaseNotes = $(git log --format="%s%n%n%b" -1 HEAD).Replace("\n", "`n")
        Update-ModuleManifest -Path $ManifestPath -ReleaseNotes $ReleaseNotes
    }
    ElseIf ($env:RELEASE_NOTES_TITLE) {
        $ReleaseNotesTitle = $env:RELEASE_NOTES_TITLE
        $ReleaseNotesBody = $env:RELEASE_NOTES_BODY.Replace("\n", "`n")
