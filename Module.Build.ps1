...
Task UpdateModuleVersion -If ($BuildNumber) -After StageFiles {
    ...
    $ManifestData = Import-LocalizedData -BaseDirectory $ModuleOutDir -FileName "$ModuleName.psd1"
    $DateInt = [Int32][DateTime]::now.ToString("yyyyMMdd")
    $ModuleVersion = [Version]$ManifestData.ModuleVersion
    $NewVersion = New-Object System.Version -ArgumentList $ModuleVersion.Major,$ModuleVersion.Minor,$DateInt,$BuildNumber
    (Get-Content $ManifestPath) -replace "ModuleVersion ?= ?['`"](\d+\.?){2,4}['`"]", "ModuleVersion = '$NewVersion'" | Set-Content $ManifestPath
}
...
Task Test -If ((Get-Module Pester -ListAvailable) -and (Test-Path $TestRootDir)) Build,{
    ...
    try {
        ...
        $TestResult = &$Exe -NonInteractive -NoProfile -Args $TestRootDir,$Testing -Command {
            param($TestRootDir, $PesterArgs)
            Import-Module Pester
            Microsoft.PowerShell.Management\Set-Location -LiteralPath $TestRootDir
...
Task Publish -If ($NuGetApiKey) Build, Test, BuildHelp, GenerateFileCatalog, {
    ...
    Publish-Module @publishParams -Force -Confirm:$false -ErrorAction Stop
}
